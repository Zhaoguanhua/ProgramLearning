PRO test_flaash_object_class_multi3
  COMPILE_OPT idl2
  ...
  FOREACH metedata,r,i DO BEGIN
    ...
    ;输出的辐射校正和大气校正文件路径
    outfilepath = strjoin([Root_outputfile_path,b[-3],b[-2],b[-1]],'\')
    outFLAASH = strjoin([Root_outputfile_path,b[-3],b[-2],'FLAASH'+b[-1]],'\')

    outdataBIL = outfilepath+'\'+strmid(file_basename(metedata),0,40)+'.dat'
    
    ;删除存在的同名文件
    IF file_test(outfilepath)EQ 1 THEN BEGIN
      FILE_DELETE,outfilepath,/RECURSIVE
    ENDIF
    
    IF file_test(outFLAASH)EQ 1 THEN BEGIN
      FILE_DELETE,outFLAASH,/RECURSIVE
    ENDIF
    ...
    IF err THEN BEGIN
      ...
      ;判断有无errlog文件，记录出错的影像，存在 在末尾添加，不存在 生成errlog.txt
      IF file_test(Root_outputfile_path) EQ 0 THEN BEGIN
        openw,lun,Root_outputfile_path+'\errlog.txt',/get_lun
      ENDIF ELSE BEGIN
        openw,lun,Root_outputfile_path+'\errlog.txt',/get_lun,/APPEND
      ENDELSE
      
      printf,lun,systime()
      printf,lun,metedata+'打开出错'
      free_lun,lun
      CONTINUE
    ENDIF
  ...
  flaash_obj = obj_new('flaash_batch',/rad_remove,/anc_delete)
  ...
  GreenRaster = ENVIRaster(Greenband,URI=Greenbandfile,NBANDS=1,DATA_IGNORE_VALUE=0)
  GreenRaster.Save
  
  RedRaster = ENVIRaster(Redband,URI=Redbandfile,NBANDS=1,DATA_IGNORE_VALUE=0)
  RedRaster.Save
  
  NirRaster = ENVIRaster(Nirband,URI=Nirbandfile,NBANDS=1,DATA_IGNORE_VALUE=0)
  NirRaster.Save
